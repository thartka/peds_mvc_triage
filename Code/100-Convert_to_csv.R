...
# remove cases with missing sex, dvtotal, rollover, pdof, or restrain use
occ_bayes %>% filter(is.na(sex)) %>% count()
occ_bayes <- occ_bayes %>% filter(!is.na(sex))

occ_bayes %>% filter(is.na(dvtotal)) %>% count()
occ_bayes <- occ_bayes %>% filter(!is.na(dvtotal))

occ_bayes  %>% filter(is.na(pdof1)) %>% count()
occ_bayes <- occ_bayes %>% filter(!is.na(pdof1))

occ_bayes %>% filter(is.na(rollover)) %>% count()
occ_bayes <- occ_bayes %>% filter(!is.na(rollover))

occ_bayes  %>% filter(is.na(chtype)) %>% count()
occ_bayes <- occ_bayes %>% filter(!is.na(chtype))

occ_bayes %>% filter(is.na(manuse)) %>% count()
occ_bayes <- occ_bayes %>% filter(!is.na(manuse))
...
occ_bayes$roll_turns <- ifelse(occ_bayes$roll_turns==98, 17, occ_bayes$roll_turns > 17)
...
occ_bayes <- occ_bayes %>% 
  select(key, year, age, sex, wgt_imp, hgt_imp, prop_restraint, no_restraint, seat_row,
         dvtotal, multicoll, pdof_front, pdof_rear, pdof_nearside, pdof_farside, rolled, roll_turns,
...
# write to file
occ_bayes %>% write.csv("./Data/NASS/NASSPeds-2000_2015.csv", row.names = FALSE)
